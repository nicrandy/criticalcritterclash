...
export function BattleGame({ onClose, scannedId }: { onClose:()=>void; scannedId?: string | null }) {
  ...
  const resolveRound = (pAct:Action, pRoll:number, aAct:Action, aRoll:number, curP:Fighter, curA:Fighter, curPS:number, curAS:number, goesFirst:'player'|'ai') => {
    ...
    if (goesFirst === 'ai') {
      ...
      schedule(() => {
        showAIHit();
        if (r.playerDefeated) { schedule(()=>finishRound(entries,r.pHpFinal,r.aHpFinal,'ai'),1000); return; }
      ...
      schedule(() => {
        showPlayerHit();
        if (r.aiDefeated) { schedule(()=>finishRound(entries,r.pHpFinal,r.aHpFinal,'player'),1000); return; }
